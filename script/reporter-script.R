...
update <- read_csv("data/datastore/MDSF_data_30 April 2025.csv", locale = locale(encoding = "Windows-1252"))
...
care_homes <- df %>%
  ...
  mutate(`Complaints_upheld_since_23_24` = sum(c_across(starts_with("Complaints_upheld_")), na.rm = TRUE)) %>% 
  mutate(`Enforcements_upheld_since_23_24` = sum(c_across(starts_with("Enforcements_issued_")), na.rm = TRUE))
...
dundee_care_homes <- care_homes %>% 
  ...
  select(-c(Complaints_upheld_2425, Complaints_upheld_2324, Enforcements_issued_2425, Enforcements_issued_2324)) %>% 
  arrange(Complaints_upheld_since_23_24) %>% 
